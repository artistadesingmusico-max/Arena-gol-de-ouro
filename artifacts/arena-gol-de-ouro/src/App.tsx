...
const addDays = (day: string, amount: number) => { const value = new Date(`${day}T12:00:00Z`); value.setUTCDate(value.getUTCDate() + amount); return value.toISOString().slice(0, 10); };
...
function BookingWidget({ facilities, initialFacilityId, initialCourt, onSuccess }: { facilities: Facility[]; initialFacilityId?: number; initialCourt?: number; onSuccess: (booking: Booking) => void }) {
  ...
  const weeks = repeatMonthly ? RECURRENCE_WEEKS : 1;
  ...
  const toggleSlot = (item: { startTime: string; endTime: string }) => setCart((current) => {
    ...
    if ((current.length + 1) * weeks > MAX_COMBO_ITEMS) return current;
  ...
  const bookingItems = useMemo(() => sortedCart.flatMap((item) => Array.from({ length: weeks }, (_, week) => ({ ...item, date: addDays(item.date, week * 7) }))).sort((a, b) => a.date.localeCompare(b.date) || a.facilityId - b.facilityId || a.court - b.court || a.startTime.localeCompare(b.startTime)), [sortedCart, weeks]);
  const totalCents = bookingItems.reduce((sum, item) => sum + priceOf(item.facilityId), 0);
  const lastRepeatDate = cart.length ? addDays(sortedCart[sortedCart.length - 1].date, (weeks - 1) * 7) : '';
  ...
  const submit = () => {
    ...
    setSubmitError('');
    createBooking.mutate({ data: { items: bookingItems.map(({ facilityId: id, court: courtNumber, date: day, startTime, endTime }) => ({ facilityId: id, court: courtNumber, date: day, startTime, endTime })), customerName: customer.name, customerPhone: customer.phone, customerEmail: customer.email, paymentMethod } }, {
      onSuccess: (created) => {
      ...
      onError: (error) => {
        ...
        if (status === 409) {
          ...
          setSubmitError(`${serverMessage ?? 'Algum horário do combo não está mais disponível.'} Volte, revise o combo e escolha outro horário.`);
        } else {
