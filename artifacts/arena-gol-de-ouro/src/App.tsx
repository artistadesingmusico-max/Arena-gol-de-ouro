...
const imageBackground = (image: string): CSSProperties => ({ '--sport-bg': `url("${image}")` } as CSSProperties);

function Brand({ compact = false }: { compact?: boolean }) {
  return <div className="flex items-center gap-3" data-testid="brand-arena">
    <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#f2c13a]/50 bg-[#f2c13a] text-[#12151c]">
      <Trophy size={21} strokeWidth={2.5} />
      <span className="absolute -bottom-1 -right-1 h-3 w-3 rounded-full border-2 border-[#0e1118] bg-[#a8e85d]" />
    </div>
    <div className={compact ? 'hidden sm:block' : ''}>
      <div className="arena-display text-[15px] font-bold uppercase leading-none tracking-[.08em] text-[#f6f0dc]">Arena</div>
      <div className="mt-1 text-[10px] font-bold uppercase leading-none tracking-[.2em] text-[#f2c13a]">Gol de Ouro</div>
    </div>
  </div>;
}
...
function Hero() {
  return <section id="topo" className="sport-section hero-glow relative min-h-[720px] overflow-hidden border-b border-white/10 bg-[#0e1118] pt-28">
    <img src={aerialImage} alt="Vista aérea da Arena Gol de Ouro ao pôr do sol" className="absolute inset-0 h-full w-full object-cover opacity-[.62] saturate-[1.15]" />
    <div className="absolute inset-0 bg-[linear-gradient(90deg,#0e1118_4%,rgba(14,17,24,.85)_35%,rgba(14,17,24,.18)_100%)]" />
    <div className="absolute inset-0 bg-[linear-gradient(0deg,#0e1118_0%,transparent_35%,rgba(14,17,24,.42)_100%)]" />
    <div className="relative mx-auto flex min-h-[592px] max-w-[1280px] items-center px-5 pb-20 lg:px-8">
      <div className="max-w-[650px]">
        ...
        <div className="mt-14 flex flex-wrap items-center gap-x-7 gap-y-3 text-xs text-[#aeb6b4]"><span className="flex items-center gap-2"><MapPin size={14} className="text-[#a8e85d]" /> Shopping de Águas Lindas</span><span className="flex items-center gap-2"><Clock3 size={14} className="text-[#f2c13a]" /> 18h — 00h</span><span className="flex items-center gap-2"><ShieldCheck size={14} className="text-[#a8e85d]" /> Ambiente seguro</span></div>
      </div>
    </div>
    <div className="absolute bottom-0 left-0 right-0 mx-auto flex max-w-[1280px] items-center justify-between px-5 pb-6 lg:px-8"><span className="text-[10px] font-bold uppercase tracking-[.22em] text-white/30">Águas Lindas • GO</span><span className="pulse-line h-px w-24 bg-[#f2c13a]" /></div>
  </section>;
}

function Intro() {
  return <section id="arena" style={imageBackground(complexImage)} className="sport-section sport-section-intro bg-[#0e1118] px-5 py-24 lg:px-8 lg:py-32">
...
function Facilities({ facilities, onSelect }: { facilities: Facility[]; onSelect: (id: number) => void }) {
  ...
  return <section id="estrutura" style={imageBackground(sandImage)} className="sport-section arena-grid border-y border-[#252d38] bg-[#111720] px-5 py-24 lg:px-8">
    <div className="mx-auto max-w-[1280px]"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="eyebrow">O palco está pronto</p><h2 className="arena-display mt-4 text-4xl font-semibold text-[#f5f0df] sm:text-5xl">Escolha seu jogo.</h2></div><p className="max-w-[330px] text-sm leading-6 text-[#89929a]">Tudo o que você precisa para chegar, jogar e sair querendo marcar o próximo.</p></div>
...
function Experience() {
  return <section style={imageBackground(muralImage)} className="sport-section sport-section-experience bg-[#0e1118] px-5 py-24 lg:px-8 lg:py-32"><div className="mx-auto grid max-w-[1280px] gap-5 md:grid-cols-12 md:grid-rows-[240px_210px]">
    <div className="group relative overflow-hidden rounded-2xl md:col-span-7 md:row-span-2"><img src={muralImage} alt="Mural esportivo da Arena Gol de Ouro" className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#0e1118]/90 via-transparent to-transparent" /><div className="absolute bottom-6 left-6"><p className="eyebrow">Identidade em cada detalhe</p><h3 className="arena-display mt-2 text-3xl font-semibold text-[#f6f0df]">Aqui, o jogo<br />também é cultura.</h3></div></div>
...
function Home() {
  ...
  return <div className="noise min-h-[100dvh] overflow-hidden bg-[#0e1118]"><SiteHeader /><main><Hero /><Intro />{facilitiesQuery.isLoading ? <section className="bg-[#111720] px-5 py-20 lg:px-8"><div className="mx-auto max-w-[1280px]"><LoadingState label="Carregando espaços da arena" /></div></section> : facilitiesQuery.isError ? <section className="bg-[#111720] px-5 py-20 lg:px-8"><div className="mx-auto max-w-[1280px]"><ErrorState onRetry={() => facilitiesQuery.refetch()} /></div></section> : <Facilities facilities={facilities} onSelect={scrollToBooking} />}<Experience /><BookingWidget facilities={facilities} initialFacilityId={initialFacilityId} onSuccess={setReservation} /><LeadCapture /></main><Footer />{reservation && <Confirmation reservation={reservation} onReset={() => setReservation(null)} />}</div>;
}
